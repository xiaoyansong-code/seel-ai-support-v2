...
- Setup Progress 点击 "Connect Ticketing System" → 打开 Settings Overlay → 自动定位到 Ticketing System 区块
...
- 文档处理完成后，**在 Team Lead 消息流中发送一条通知**
...
- Setup Progress 点击 "Configure Agent" → 打开 Settings Overlay → 自动定位到 Configure Agent 区块

### 5.2 首次配置 vs 后续编辑

| 场景 | Rep Name | Personality | 按钮文案 | 完成后行为 |
|------|----------|-------------|----------|-----------|
| 首次（未 Hire） | 空，需用户填写 | 未选择，需用户选择 | "Hire Rep" | 关闭 Settings，回到 Agents 首页 |
...
**Identity**
- Rep Name（文本输入）
- Personality（单选：Friendly / Professional / Casual / Customize）
  - 每个选项下方显示描述 + 示例消息
